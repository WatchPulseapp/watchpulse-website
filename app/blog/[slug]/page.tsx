...
// Helper function to get blog from database
async function getBlogFromDB(slug: string): Promise<BlogPost | null> {
  try {
    await connectDB();
...
// Blog post data with enhanced content (static/hardcoded)
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  // First try database, then fall back to hardcoded
...
export default async function BlogPost({ params }: PageProps) {
  ...
  const articleSchema = {
    ...
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://watchpulseapp.com/blog/${slug}`
    },
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": "Blog",
        "item": "https://watchpulseapp.com/blog"
      },
      {
        "@type": "ListItem",
        "position": 3,
        "name": post.title,
        "item": `https://watchpulseapp.com/blog/${slug}`
      }
    ]
  };
      ...
      <Container className="py-20">
        ...
        <article className="max-w-4xl mx-auto">
          ...
          <div className="flex items-center gap-2 mb-6">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-brand-primary/20 rounded-full text-brand-primary text-sm font-semibold border border-brand-primary/30">
          ...
          {/* Social Sharing */}
          <div className="flex items-center gap-4 mb-12 pb-8 border-b border-brand-primary/10">
            ...
            <div className="flex items-center gap-2">
              {/* Twitter/X Share */}
              <a
                href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`https://watchpulseapp.com/blog/${slug}`)}&via=watchpulseapp`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2 bg-background-card rounded-lg border border-brand-primary/20 hover:border-brand-primary/50 hover:bg-brand-primary/10 transition-all group"
                title="Share on X (Twitter)"
              >
                <svg viewBox="0 0 24 24" className="w-4 h-4 text-brand-primary group-hover:text-brand-accent fill-current">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              {/* Facebook Share */}
              <a
                href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://watchpulseapp.com/blog/${slug}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2 bg-background-card rounded-lg border border-brand-primary/20 hover:border-brand-primary/50 hover:bg-brand-primary/10 transition-all group"
                title="Share on Facebook"
              >
                <svg className="w-4 h-4 text-brand-primary group-hover:text-brand-accent fill-current" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
              </a>
              {/* LinkedIn Share */}
              <a
                href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(`https://watchpulseapp.com/blog/${slug}`)}&title=${encodeURIComponent(post.title)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2 bg-background-card rounded-lg border border-brand-primary/20 hover:border-brand-primary/50 hover:bg-brand-primary/10 transition-all group"
              ...
              {/* Reddit Share */}
              <a
                href={`https://reddit.com/submit?url=${encodeURIComponent(`https://watchpulseapp.com/blog/${slug}`)}&title=${encodeURIComponent(post.title)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2 bg-background-card rounded-lg border border-brand-primary/20 hover:border-brand-primary/50 hover:bg-brand-primary/10 transition-all group"
              ...
              {/* WhatsApp Share */}
              <a
                href={`https://wa.me/?text=${encodeURIComponent(post.title + ' - https://watchpulseapp.com/blog/' + slug)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2 bg-background-card rounded-lg border border-brand-primary/20 hover:border-brand-primary/50 hover:bg-brand-primary/10 transition-all group"
